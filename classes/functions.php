 ...
 * @return Kurs[]|null
 */
function get_all_kurse($only_active=false){
	global $wpdb;
	$ret = array();
	$i = -1;

	if($obj = $wpdb->get_results("SELECT id FROM ".$wpdb->prefix."cw_kurse ".($only_active?"WHERE show_front=1 OR id=1":"")." ORDER BY id DESC")){
